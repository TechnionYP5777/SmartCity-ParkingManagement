...
public class MyDetails extends AbstractWindow {
	...
	public void display(final Stage primaryStage, final WindowEnum prevWindow, final ArrayList<Label> newLabels,
			final ArrayList<Label> newValues, final ArrayList<AbstractWindow> prevWindows) {
		window = primaryStage;
		...
		if (newLabels != null) {
			...
			final Label eMail = new Label("user@gmail.com");
			labels.add(new Label("eMail:"));
			values.add(eMail);

			final Label username = new Label("AwesomeUser");
			labels.add(new Label("Username:"));
			values.add(username);

			final Label carNumber = new Label("123456789");
